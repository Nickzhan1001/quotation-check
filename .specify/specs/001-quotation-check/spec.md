...
### 使用者故事 4 - 功能建議清單來源標記（優先級：P3）

使用者看到的「功能建議清單」每一筆項目都包含 `type` 欄位，用以標示資料來源。

**為何是此優先級**：用於區分原始規劃與 AI 建議，便於追溯與篩選，不影響既有功能。

**獨立測試**：檢視建議清單資料，確認每筆項目皆含 `type`，且值僅限 `original` 或 `agent_advice`。

**驗收情境**：

1. **假設** 系統產生建議清單，**當** 顯示任一筆建議，**則** `type` 欄位存在且值為 `original` 或 `agent_advice`。
...
- **FR-013**：功能建議清單中的每筆項目 MUST 包含 `type` 欄位，值僅允許 `original` 或 `agent_advice`，且此欄位僅作來源標記，不影響既有功能邏輯。
...
- **SC-005**：功能建議清單每筆項目都含 `type`，且值皆為 `original` 或 `agent_advice`。
